...
executor. Trigger whenever you are about to search files, grep for text,
...
Commander, or through an MCP shell bridge. What _does_ change between
environments — `$PATH` inheritance, which shell `/bin/sh` points to, pager
behavior, GNU-vs-BSD flag differences.
...
| Search file _contents_             | `rg` (ripgrep) | `grep`           | `rg --json 'pattern'`                         |
| Search/rewrite by code _structure_ | `ast-grep`     | `grep`+`sed`     | `ast-grep -p 'pat' -l python`                 |
...
- **`fd`** — you know something about the _filename, path, or type_
  ("find all `test_*.py`").
- **`rg`** — you want _text/content_ anywhere, as a fast first pass. Use `-l`
  to get just filenames to feed the next step.
- **`ast-grep`** — you care about _code structure_, not text (refactors;
...
**General Rule:** locate with `rg`/`fd`, transform with `ast-grep`/`sd`.
...
- `sd -p 'foo' 'bar' file` — **preview** find/replace; drop `-p` to apply.
  For code _identifiers_ use ast-grep instead (sd clobbers substrings).
...
wrapper differs: bash tool process tools work directly.
If you run commands through the **LocalREPL MCP bridge**, also
...
The notes below cover where the _environment around_ the tools still bites.
...
- **Debian/Ubuntu rename `fd`→`fdfind`, `bat`→`batcat`** when installed via
  `apt`. Installed via `cargo` for canonical names.
