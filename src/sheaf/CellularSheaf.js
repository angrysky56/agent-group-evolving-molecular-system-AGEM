...
export class CellularSheaf {
    ...
    _validateEdge(edge) {
        ...
        if (!this._vertices.has(edge.sourceVertex)) {
            throw new Error(`CellularSheaf: edge '${edge.id}' references non-existent source vertex '${edge.sourceVertex}'`);
        }
        if (!this._vertices.has(edge.targetVertex)) {
            throw new Error(`CellularSheaf: edge '${edge.id}' references non-existent target vertex '${edge.targetVertex}'`);
        }
    ...
    _validateRestrictionMap(map, edge, expectedSourceDim, role) {
        ...
        if (sourceDim !== expectedSourceDim) {
            throw new Error(`CellularSheaf: dimension mismatch on edge '${edge.id}' ${role}Restriction: ` +
    ...
    laplacianComputer = null;
    getLaplacianComputer() {
...
//# sourceMappingURL=CellularSheaf.js.map