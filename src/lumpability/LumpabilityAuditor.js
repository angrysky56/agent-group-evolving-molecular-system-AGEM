...
export class LumpabilityAuditor extends EventEmitter {
    ...
    #history = [];
    #maxHistorySize = 100;
    constructor(embedder, tokenCounter, config) {
        super();
     ...
     */
    async audit(summaryNode, sourceEntries, escalationLevel) {
        // Step 1: Extract source texts.
        const sourceTexts = sourceEntries.map((e) => e.content);
        // Step 2: Compute source entropy profile.
        const sourceProfile = await computeEntropyProfile(sourceTexts, this.#embedder, this.#tokenCounter);
        // Step 3: Compute summary entropy profile.
        // Treat summary as a single text for embedding.
        const summaryProfile = await computeEntropyProfile([summaryNode.content], this.#embedder, this.#tokenCounter);
        // Step 4: Entropy preservation ratio.
        const entropyPreservationRatio = sourceProfile.entropy > 1e-12
            ? summaryProfile.entropy / sourceProfile.entropy
            : NaN;
        // Step 5: Centroid similarity.
        const centroidSimilarity = computeCentroidSimilarity(sourceProfile, summaryProfile);
        // Step 6: Select threshold for this escalation level.
        const threshold = this.#getThreshold(escalationLevel);
        // Step 7: Classify.
        const classification = this.#classify(entropyPreservationRatio, centroidSimilarity, threshold);
        // Step 8: Build result.
        const result = {
     ...
     *   - otherwise → weak
     */
    #classify(ratio, centroidSim, threshold) {
        if (isNaN(ratio))
            return "degenerate";
     ...
     */
    getWeakCompressionRate() {
        if (this.#history.length === 0)
            return 0;
        const weakCount = this.#history.filter((r) => r.classification === "weak").length;
     ...
     */
    getAverageEntropyPreservation() {
        const valid = this.#history.filter((r) => !isNaN(r.entropyPreservationRatio));
        if (valid.length === 0)
            return NaN;
        const sum = valid.reduce((acc, r) => acc + r.entropyPreservationRatio, 0);
...
//# sourceMappingURL=LumpabilityAuditor.js.map