 ...
 */
export function formatTaskForWorker(task) {
    // Validate: functions and symbols are not serializable
    if (typeof task.id !== "string" || task.id.trim() === "") {
        throw new Error(`Task ID must be a non-empty string; got: ${JSON.stringify(task.id)}`);
    }
    ...
    let clonedPayload;
    try {
        clonedPayload = JSON.parse(JSON.stringify(task.payload));
    }
    catch (err) {
        throw new Error(`Task payload for task '${task.id}' is not serializable: ${String(err)}`);
    }
    const clonedMetadata = task.metadata !== undefined
 ...
 */
export async function llm_map(tasks, workerScriptPath, poolSize = 4) {
    ...
    const contextData = contextStore
        ? Object.fromEntries(contextStore)
        : {};
    // Spawn a fresh pool of workers for this call
    const workers = Array.from({ length: effectivePoolSize }, () => new Worker(workerScriptPath));
    try {
            ...
            return new Promise((resolve) => {
                ...
                try {
                    ...
                    resolve({
                        taskId: task.id,
                        success: false,
                        error: formatErr instanceof Error
                            ? formatErr
...
//# sourceMappingURL=llm_map.js.map