...
export class ObstructionHandler {
     ...
     */
    constructor(eventBus, gapDetector, tnaGraph, config) {
        ...
        this.#obstructionHandler = (event) => {
            this.#onObstructionDetected(event);
        };
     ...
     */
    async #processSingleObstruction(obstruction) {
        ...
        const filledEvent = {
            type: "orch:obstruction-filled",
            ...gapFillResult,
        };
        // Emit as unknown/any since orch:obstruction-filled is not in AnyEvent union
        await this.#eventBus.emit(filledEvent);
        console.log(`[ORCH-OBSTRUCTION] Filled obstruction from iteration ${obstruction.iteration}: ` +
     ...
     */
    async #processVdWSpawning(obstruction) {
        if (!this.#vdwSpawner)
            return;
        const gaps = this.#gapDetector.findGaps();
        const spawnedAgents = await this.#vdwSpawner.evaluateAndSpawn(gaps, obstruction.h1Dimension, obstruction.iteration);
        if (spawnedAgents.length > 0) {
            console.log(`[ORCH-OBSTRUCTION] Spawned ${spawnedAgents.length} VdW agents ` +
     ...
     */
    async #runGapDetectorAgent(agent, obstruction) {
     ...
     */
    #integrateGapFillResults(results) {
        // Add new entity nodes to TNA graph via ingestTokens()
        if (results.entitiesAdded.length > 0) {
            this.#tnaGraph.ingestTokens(results.entitiesAdded);
            console.log(`[ORCH-OBSTRUCTION] Integrated ${results.entitiesAdded.length} new entities from gap fill`);
        }
        ...
        if (results.relationsAdded.length > 0) {
            console.log(`[ORCH-OBSTRUCTION] ${results.relationsAdded.length} relations noted ` +
     ...
     * No-op if no VdW spawner is configured (backward compatible).
     */
    updateH1ForSpawner(h1Dimension) {
...
//# sourceMappingURL=ObstructionHandler.js.map