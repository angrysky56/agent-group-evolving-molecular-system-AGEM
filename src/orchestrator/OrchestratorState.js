...
export class OrchestratorStateManager {
    #currentState = OrchestratorState.NORMAL;
    #h1ObstructionThreshold;
    #h1CriticalThreshold;
    #eventBus;
     ...
     */
    constructor(eventBus, h1ObstructionThreshold = 2, h1CriticalThreshold = 5) {
     ...
     */
    updateMetrics(h1Dimension) {
        const current = this.#currentState;
        const obs = this.#h1ObstructionThreshold;
        const crit = this.#h1CriticalThreshold;
        if (current === OrchestratorState.NORMAL) {
            if (h1Dimension >= crit) {
                this.#setState(OrchestratorState.CRITICAL, `H^1=${h1Dimension} exceeds critical threshold ${crit}`, h1Dimension);
            }
            else if (h1Dimension >= obs) {
                this.#setState(OrchestratorState.OBSTRUCTED, `H^1=${h1Dimension} crosses obstruction threshold ${obs}`, h1Dimension);
            }
            // else: no transition
        }
        else if (current === OrchestratorState.OBSTRUCTED) {
            if (h1Dimension >= crit) {
                this.#setState(OrchestratorState.CRITICAL, `H^1=${h1Dimension} escalates to critical`, h1Dimension);
            }
            else if (h1Dimension < obs) {
                this.#setState(OrchestratorState.NORMAL, `H^1=${h1Dimension} returns below obstruction threshold ${obs}`, h1Dimension);
            }
     ...
     */
    #setState(newState, reason, h1Metric) {
