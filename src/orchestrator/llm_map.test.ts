...
describe("T12: formatTaskForWorker validates task serialization", () => {
  ...
  it("task with function payload throws (functions are not serializable)", () => {
    ...
    expect(
      (formatted.payload as Record<string, unknown>)["fn"],
    ).toBeUndefined();
  });
});
...
describe("T14: Context propagation via AsyncLocalStorage.run() nesting", () => {
  ...
  it("different llm_map calls with different contexts are isolated", async () => {
    ...
    const [results1, results2] = await Promise.all([
      ...
      contextStorage.run(ctx2, () =>
        llm_map(
          tasks.map((t) => ({ ...t, id: t.id + "-2" })),
          MOCK_WORKER_PATH,
          1,
        ),
      ),
    ...
    const ctx1Data = (results1[0].result as Record<string, unknown>)[
      "_context"
    ] as Record<string, unknown>;
    const ctx2Data = (results2[0].result as Record<string, unknown>)[
      "_context"
    ] as Record<string, unknown>;
