...
function createSpawner(config?: Partial<VdWSpawnerConfig>): {
  ...
  eventBus.subscribe("orch:vdw-agent-complete", (e) => {
    events.push(e);
  });
...
describe("VdWAgentSpawner", () => {
  ...
  describe("Regime gating", () => {
    ...
    it('T19: Regime change from "stable" to "critical" → spawning resumes', async () => {
      ...
      const stableAgents = await spawner.evaluateAndSpawn(
        [makeGap(0, 1)],
        5,
        1,
      );
      ...
      // Switch to critical
      spawner.updateRegime("critical");
      const criticalAgents = await spawner.evaluateAndSpawn(
        [makeGap(0, 2)],
        5,
        2,
      );
  ...
  describe("Event emission", () => {
    it('T32: "orch:vdw-agent-spawned" emitted for each spawned agent', async () => {
      ...
      const spawnedEvents = events.filter(
        (e) => e.type === "orch:vdw-agent-spawned",
      );
