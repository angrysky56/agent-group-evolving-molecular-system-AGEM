...
// ---------------------------------------------------------------------------
import { CohomologyAnalyzer, buildFlatSheaf, } from "../sheaf/index.js";
// ---------------------------------------------------------------------------
// LCM module imports
// ---------------------------------------------------------------------------
...
import { OrchestratorStateManager, } from "./OrchestratorState.js";
...
export class Orchestrator {
    ...
    #wireSocEvents() {
        ...
        this.socTracker.on("regime:classification", (event) => {
            void this.eventBus.emit(event);
        });
        this.eventBus.subscribe("regime:classification", (event) => {
            const regimeEvent = event;
            this.obstructionHandler.updateRegime(regimeEvent.regime);
        });
        this.socTracker.on("soc:system1-early-convergence", (event) => {
            void this.eventBus.emit(event);
        });
    }
    #wireTnaEvents() {
        this.tnaCentrality.on("tna:centrality-change-detected", (event) => {
            void this.eventBus.emit(event);
        });
        this.tnaCentrality.on("tna:topology-reorganized", (event) => {
            void this.eventBus.emit(event);
        });
        this.eventBus.subscribe("regime:classification", (event) => {
            const regimeEvent = event;
        ...
        this.tnaLayout.on("tna:layout-updated", (event) => {
            void this.eventBus.emit(event);
        });
        this.eventBus.subscribe("regime:classification", (event) => {
            const regimeEvent = event;
    ...
    #wireStateEvents() {
        this.eventBus.subscribe("orch:state-changed", (event) => {
            ...
            console.log(`[ORCH] Iteration ${this.#iterationCounter}: State transition to ` +
     ...
     */
    #wireLumpabilityEvents() {
        this.lumpabilityAuditor.on("lumpability:audit-complete", (event) => {
            void this.eventBus.emit(event);
        });
        this.lumpabilityAuditor.on("lumpability:weak-compression", (event) => {
            void this.eventBus.emit(event);
        });
    }
    /**
    ...
    #registerDefaultSubscribers() {
        this.eventBus.subscribe("sheaf:consensus-reached", (event) => {
            const e = event;
            console.log(`[ORCH] Iteration ${this.#iterationCounter}: Sheaf consensus at h0=${e.h0Dimension ?? "?"}`);
        });
        this.eventBus.subscribe("sheaf:h1-obstruction-detected", (event) => {
            const e = event;
            console.log(`[ORCH] Iteration ${this.#iterationCounter}: H^1 obstruction dim=${e.h1Dimension ?? "?"}`);
        });
        this.eventBus.subscribe("soc:metrics", (event) => {
            const e = event;
            console.log(`[ORCH] Iteration ${e.iteration ?? "?"}: SOC metrics ` +
        ...
        this.eventBus.subscribe("phase:transition", (event) => {
            const e = event;
            console.log(`[ORCH] Iteration ${e.iteration ?? "?"}: PHASE TRANSITION detected ` +
                `(r=${(e.correlationCoefficient ?? 0).toFixed(3)})`);
        });
        this.eventBus.subscribe("lumpability:weak-compression", (event) => {
            const e = event;
            console.log(`[ORCH] WEAK LUMPABILITY at ${e.summaryNodeId ?? "?"} ` +
        ...
        this.eventBus.subscribe("soc:system1-early-convergence", (event) => {
            const e = event;
            console.log(`[ORCH] SYSTEM 1 OVERRIDE detected at iteration ${e.iteration ?? "?"}: ` +
    ...
    async runReasoning(prompt) {
        ...
        console.log(`[ORCH] Iteration ${this.#iterationCounter}: TNA processed ` +
        ...
        if (this.tnaGraph.order > 0) {
            ...
            console.log(`[ORCH] Iteration ${this.#iterationCounter}: Communities detected: ` +
        ...
        const cohomologyResult = this.cohomologyAnalyzer.analyze(this.sheaf, this.#iterationCounter);
        console.log(`[ORCH] Iteration ${this.#iterationCounter}: Sheaf analysis: ` +
        ...
        console.log(`[ORCH] Iteration ${this.#iterationCounter}: Price equation — ` +
     ...
     */
    async auditCompaction(summaryNode, sourceEntries, escalationLevel) {
        await this.lumpabilityAuditor.audit(summaryNode, sourceEntries, escalationLevel);
    }
    // -------------------------------------------------------------------------
    ...
    #initTna() {
        ...
        this.tnaCatalystGenerator = new CatalystQuestionGenerator(this.tnaGraph, this.tnaCentrality);
        this.tnaLayout = new LayoutComputer(this.tnaGraph);
    }
    #initSoc() {
        this.socTracker = new SOCTracker({ correlationWindowSize: 10 });
    }
    #initLumpability(embedder) {
        const tokenCounter = new GptTokenCounter();
        this.lumpabilityAuditor = new LumpabilityAuditor(embedder, tokenCounter);
    }
    #initEvolution() {
        this.priceEvolver = new PriceEvolver(this.tnaGraph.getGraph());
    }
    #initOrchestration() {
        const vdwSpawner = new VdWAgentSpawner(this.eventBus);
        this.obstructionHandler = new ObstructionHandler(this.eventBus, this.tnaGapDetector, this.tnaGraph, { agentPoolSize: 4, vdwSpawner });
    }
    // -------------------------------------------------------------------------
    // Private: SOCInputs helpers
    // -------------------------------------------------------------------------
    /** Embed any TNA graph nodes not yet in the cache. */
    async #updateNodeEmbeddings(graph) {
        const nodesToEmbed = [];
        ...
        if (nodesToEmbed.length > 0) {
            console.log(`[ORCH] Embedded ${this.#nodeEmbeddings.size} TNA nodes (${nodesToEmbed.length} new)`);
        }
    }
...
//# sourceMappingURL=ComposeRootModule.js.map