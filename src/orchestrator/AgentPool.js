...
export class AgentPool {
     ...
     */
    constructor(agentFactory, config) {
        this.#config = config;
        this.#agents = [];
    ...
    async initialize() {
        ...
        this.#heartbeatTimer = setInterval(() => void this.#runHeartbeat(), this.#config.heartbeatIntervalMs);
    }
    /**
    ...
    async #runHeartbeat() {
        const promises = this.#agents
            ...
            .map((agent) => Promise.race([
            agent.heartbeat(),
            new Promise((_, reject) => setTimeout(() => reject(new Error(`Heartbeat timeout for agent ${agent.id}`)), this.#config.heartbeatTimeoutMs)),
        ]).catch((err) => {
