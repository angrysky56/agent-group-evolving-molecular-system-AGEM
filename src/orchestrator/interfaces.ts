...
// ---------------------------------------------------------------------------
// Meta-Orchestrator (Phase 7) types
// ---------------------------------------------------------------------------

/**
 * TopologicalType — the architectural pattern chosen for task execution.
 */
export type TopologicalType = "sequential_workflow" | "planning_react" | "react" | "multi_agent";

/**
 * TopologicalManifest — the structured blueprint generated by the AgenticDesignPicker.
 */
export interface TopologicalManifest {
  /** The chosen architectural pattern. */
  readonly topologyType: TopologicalType;

  /** Tools required for this task. */
  readonly requiredTools: string[];

  /** Whether to append a reflection loop (critique-refine pass). */
  readonly reflectionEnabled: boolean;

  /** Maximum iterations for the reasoning loop. */
  readonly maxIterations: number;

  /** Specific roles for a multi-agent topology (if selected). */
  readonly specialistRoles?: string[];

  /** Inhibitory constraints enforced by the picker. */
