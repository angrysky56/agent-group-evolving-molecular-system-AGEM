...
export function pearsonCorrelation(x: number[], y: number[]): number {
  ...
  if (denominator < 1e-12) return 0; // zero variance → no correlation defined
...
export function linearSlope(values: number[]): number {
  ...
  if (Math.abs(denominator) < 1e-12) return 0; // degenerate (all indices equal — impossible for n>=2)
