 ...
 */
function buildCompleteGraphEdges(
  n: number,
): ReadonlyArray<{ source: number; target: number; weight: number }> {
 ...
 */
function buildOrthogonalEmbeddings(
  nodeIds: string[],
  dim: number,
...
describe("CDP computation (SOC-03)", () => {
  it("T-CDP-01: CDP equals vonNeumannEntropy minus embeddingEntropy", () => {
    ...
    const expectedVNE = vonNeumannEntropy(
      inputs.nodeCount,
      inputs.edges as Array<{ source: number; target: number; weight: number }>,
    );
    ...
    expect(
      Math.abs(
        metrics.cdp - (metrics.vonNeumannEntropy - metrics.embeddingEntropy),
      ),
    ).toBeLessThan(1e-10);
  });
...
describe("Surprising edge ratio (SOC-04)", () => {
  it("T-SE-01: All intra-community new edges yield ratio = 0 (ROADMAP SC-3)", () => {
    ...
    const newEdges = [{ source: "a", target: "b", createdAtIteration: 5 }];
  ...
  it("T-SE-02: All cross-community + low similarity new edges yield ratio = 1", () => {
    ...
    const newEdges = [{ source: "a", target: "b", createdAtIteration: 7 }];
  ...
  it("T-SE-03: Cross-community but HIGH similarity is NOT surprising", () => {
    ...
    const newEdges = [{ source: "a", target: "b", createdAtIteration: 8 }];
  ...
  it("T-SE-05: Per-iteration isolation — prior iteration edges do not count (Pitfall 3 guard)", () => {
    ...
    const metrics2 = tracker.computeAndEmit(
      buildSyntheticInputs({
        nodeCount: 4,
        embeddings,
        communityAssignments,
...
describe("Phase transition detection (SOC-05)", () => {
  ...
  function buildIterationInputs(opts: {
    ...
    if (embeddingDiversity === "diverse") {
      ...
      for (let k = 0; k < nodeCount; k++) {
        embeddings.set(
          String(k),
          new Float64Array(nodeCount).fill(1 / Math.sqrt(nodeCount)),
        );
      }
    }
  ...
  it("T-PT-01: Synthetic trajectory with known sign change fires phase:transition", () => {
    ...
    function buildBlendedEmbeddings(
      t: number,
    ): ReadonlyMap<string, Float64Array> {
  ...
  it("T-PT-02: Stable trajectory (no sign change) does not fire phase:transition", () => {
    ...
    for (let i = 1; i <= 15; i++) {
      tracker.computeAndEmit(
        buildSyntheticInputs({
          nodeCount,
          edges: buildCompleteGraphEdges(nodeCount) as Array<{
            source: number;
            target: number;
            weight: number;
          }>,
  ...
  it("T-PT-04: Window size is configurable — smaller window may detect transition earlier", () => {
    ...
    trackerSmall.on(
      "phase:transition",
      (event: { centeredAtIteration: number }) => {
    ...
    const smallConfig = trackerSmall as unknown as {
      _config?: { correlationWindowSize: number };
    };
  ...
  it("T-EV-05: getMetricsTrend(5) returns mean and slope for monotonically increasing VN entropy", () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      tracker.computeAndEmit(
        buildSyntheticInputs({
          nodeCount: n,
          edges: buildCompleteGraphEdges(n) as Array<{
            source: number;
            target: number;
            weight: number;
          }>,
...
describe("Phase 6: Regime validation integration", () => {
    ...
    for (let i = 1; i <= 20; i++) {
      ...
      tracker.computeAndEmit({
        ...
        communityAssignments: new Map(
          Array.from({ length: N }, (_, k) => [String(k), 0]),
        ) as ReadonlyMap<string, number>,
        newEdges: [],
        iteration: i,
      });
  ...
  it("T-INT-9: computeAndEmit() return type unchanged (backward compatible — still returns SOCMetrics)", () => {
    const tracker = new SOCTracker();
    const result = tracker.computeAndEmit(
      buildSyntheticInputs({ iteration: 1 }),
    );
