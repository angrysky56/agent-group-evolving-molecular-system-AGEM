...
describe('CooccurrenceGraph', () => {
  ...
  it('T8: createdAtIteration attribute is set on edges at ingest time', () => {
    ...
    g.forEachEdge((_edge: unknown, attrs: Record<string, unknown>, source: string, target: string) => {
      const iter = attrs['createdAtIteration'] as number;
      if (['apple', 'banana', 'cherry'].includes(source) && ['apple', 'banana', 'cherry'].includes(target)) {
        expect(iter).toBe(3);
        foundIter3 = true;
      }
      if (['dragon', 'elderberry', 'fig'].includes(source) && ['dragon', 'elderberry', 'fig'].includes(target)) {
        expect(iter).toBe(7);
        foundIter7 = true;
      }
    });
