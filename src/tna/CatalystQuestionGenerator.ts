...
const QUESTION_TEMPLATES = [
  (a: string, b: string) => `How does ${a} relate to ${b}?`,
  (a: string, b: string) => `What concept bridges ${a} and ${b}?`,
  (a: string, b: string) => `In what context would ${a} and ${b} co-occur?`,
] as const;
...
export class CatalystQuestionGenerator {
  ...
  readonly #maxRepresentativeNodes: number;
   ...
   */
  #getRepresentativeNodes(communityId: number, count: number): TextNode[] {
   ...
   */
  #computeSemanticDistance(nodesA: TextNode[], nodesB: TextNode[]): number {
    if (nodesA.length === 0 || nodesB.length === 0) {
