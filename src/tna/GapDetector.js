...
export class GapDetector {
    ...
    static DENSITY_THRESHOLD = 0.2;
    constructor(cooccurrenceGraph, louvainDetector, centralityAnalyzer) {
     ...
     */
    #evaluateCommPair(graph, commA, commB, membersA, membersB) {
        // Count inter-community edges.
        let interEdges = 0;
        const interEdgeNodes = new Set();
        ...
        if (density >= _a.DENSITY_THRESHOLD)
            return null;
     ...
     */
    getGapBetween(communityA, communityB) {
        const gaps = this.findGaps();
        for (const gap of gaps) {
            if ((gap.communityA === communityA && gap.communityB === communityB) ||
     ...
     */
    #computeShortestPathLength(graph, membersA, membersB) {
     ...
     */
    #estimateModularityDelta(graph, membersA, membersB) {
     ...
     */
    #identifyBridgeNodes(interEdgeNodes) {
        const candidates = [];
        ...
        return candidates.slice(0, topN).map((c) => c.nodeId);
    }
}
_a = GapDetector;
