...
function buildThreeClusterGraph(): {
  ...
  g.forEachEdge((edge, _attrs, source, target) => {
    ...
    const isInterClique = (srcInA && (dstInB || dstInC)) || (srcInB && (dstInA || dstInC)) || (srcInC && (dstInA || dstInB));
    if (!isInterClique) return;

    const isBridgeAB = (source === 'a1' && target === 'b1') || (source === 'b1' && target === 'a1');
    const isBridgeBC = (source === 'b1' && target === 'c1') || (source === 'c1' && target === 'b1');
...
describe('GapDetector', () => {
  ...
  it('T16b: two-clique bridge graph has exactly one gap', () => {
    const detector = new GapDetector(twoCliqueGraph, twoCliqueLouvain, twoCliqueCentrality);
    const gaps = detector.findGaps();
  ...
  it('T19b: running findGaps twice returns the same result', () => {
    ...
    for (let i = 0; i < gaps1.length; i++) {
      ...
      expect(gap1.interCommunityDensity).toBeCloseTo(gap2.interCommunityDensity, 6);
    }
  });
});
