...
function makeSummaryNode(
  ...
  intermediateCompressions: Array<{
    level: number;
    content: string;
    childIds: string[];
  }> = [],
...
describe("T12c: lcm_expand yields original entries last", () => {
  it('after summary and compressions, original entries are yielded with kind="entry"', async () => {
    ...
    expect((entryResults[0] as Record<string, unknown>)["content"]).toBe(
      "First original entry",
    );
    expect((entryResults[1] as Record<string, unknown>)["content"]).toBe(
      "Second original entry",
    );
    expect((entryResults[2] as Record<string, unknown>)["content"]).toBe(
      "Third original entry",
    );
  });
});
