...
export class ContextDAG {
   ...
   */
  #detectCycle(node: SummaryNode): void {
    ...
    while (stack.length > 0) {
      ...
      if (currentId === targetId) {
        throw new Error(
          `ContextDAG: cycle detected — node '${targetId}' is reachable from its own intermediateCompressions childIds`,
        );
      }
