...
import { GptTokenCounter, MockEmbedder, type IEmbedder, type LCMEntry } from './interfaces.js';
...
describe('T4b: tokenCount matches gpt-tokenizer', () => {
  it('entry.tokenCount === encode(content).length', () => {
    const store = freshStore();
    const content = 'Testing token counting with multiple words and punctuation!';
    const entry = store.append(content);
...
describe('T1d: LCMClient.append() caches embedding at append time', () => {
  it('store has entry, cache has embedding, embedder called once', async () => {
    ...
    const client = new LCMClient(store, cache as unknown as import('./EmbeddingCache.js').EmbeddingCache, mockEmbedder);
