...
export class PriceEvolver {
    ...
    onCohomologyUpdate(h1Dimension) {
        const delta = this.#previousH1 - h1Dimension;
        if (delta > 0) {
            // H¹ decreased — reward all current edges
            this.#applyGlobalFitness(this.#config.h1ReductionFitness * delta, `H¹ reduced by ${delta}`);
        }
    ...
    onSOCMetrics(cdp) {
        ...
        if (delta > 0) {
            ...
            this.#applyRecentEdgeFitness(this.#config.cdpIncreaseFitness, `CDP increased by ${delta.toFixed(4)}`);
        }
        this.#previousCDP = cdp;
    }
    /** Reward edges near bridge nodes when a gap closes. */
    onGapClosure(bridgeNodes) {
    ...
    onWeakLumpability(sourceEntryIds) {
        // Penalize recently-created edges as they contributed to weak compression
        this.#applyRecentEdgeFitness(this.#config.weakLumpabilityPenalty, `weak lumpability (${sourceEntryIds.length} entries)`);
    }
    // ─── Core Evolution Step ───
    /**
    ...
    // ─── Private Helpers ───
    #addFitness(edgeKey, delta, reason) {
    ...
    #applyGlobalFitness(delta, reason) {
    ...
    #applyRecentEdgeFitness(delta, reason) {
        this.#graph.forEachEdge((edgeKey, attrs) => {
            const created = attrs.createdAtIteration;
...
//# sourceMappingURL=PriceEvolver.js.map