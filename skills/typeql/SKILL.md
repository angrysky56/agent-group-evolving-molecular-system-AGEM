---
name: "typeql"
description: "TypeQL language reference for TypeDB 3.x — schema definition, CRUD, query pipelines, functions. Use whenever writing or reviewing TypeQL for the AGEM claim store (schema/claims.tql). TypeDB 3.x is a rewrite; 2.x syntax from memory is wrong."
---

> **Provenance:** vendored from [typedb/typedb-skills](https://github.com/typedb/typedb-skills) `typeql.md`.
> TypeDB publish their own benchmark for this file: TypeQL pass rate rises from
> 23–43% (unaided) to 86–96% with this reference plus `typeql-check`. Do not
> write TypeQL for AGEM from memory — 3.x changed the root types (`thing` is
> gone), the insert syntax (`links (role: $x)`), and the pipeline model.
>
> **AGEM's schema lives at `schema/claims.tql`.** Validate changes by loading
> them against a scratch database rather than by inspection:
> `typedb console --address 127.0.0.1:1729 --tls-disabled --username admin \`
> `  --command "transaction schema <db>" --command "source schema/claims.tql" --command "commit"`
> Note `--command` takes *console* commands, not raw TypeQL: inline queries fail
> with "Missing argument 1: query", which looks like a schema rejection and is
> not one. Put queries in a file and `source` it.
