...
export class ScenarioService {
  ...
  > {
    ...
    for (const f of files) {
      ...
      try {
        ...
        const filePath = path.normalize(path.join(SCENARIOS_DIR, safe));
  ...
  async loadScenario(id: string): Promise<Scenario | null> {
    ...
    const filePath = path.normalize(path.join(SCENARIOS_DIR, `${safe}.json`));
    if (!filePath.startsWith(SCENARIOS_DIR + path.sep)) {
      return null;
    }
    try {
  ...
  async saveScenario(scenario: Scenario): Promise<string> {
    ...
    if (
      ...
      !/^[a-zA-Z0-9_-]+$/.test(scenario.id)
    ) {
      throw new Error("Invalid scenario ID format");
    }
    const safe = path.basename(scenario.id);
    const filePath = path.normalize(path.join(SCENARIOS_DIR, `${safe}.json`));
    if (!filePath.startsWith(SCENARIOS_DIR + path.sep)) {
      throw new Error("Invalid scenario destination path");
    }
  ...
  async saveResult(result: ScenarioResult): Promise<string> {
    ...
    if (
      ...
      !/^[a-zA-Z0-9_-]+$/.test(result.scenario_id)
    ) {
      throw new Error("Invalid scenario ID in result");
    }
    const safeId = path.basename(result.scenario_id);
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    const fileName = `${safeId}_${timestamp}.json`;
    const filePath = path.normalize(path.join(RESULTS_DIR, fileName));
    if (!filePath.startsWith(RESULTS_DIR + path.sep)) {
      throw new Error("Invalid result destination path");
    }
  ...
  async listResults(): Promise<
    Array<{ file: string; scenario_id: string; date: string }>
  > {
    await this.init();
    ...
    for (const f of files) {
      ...
      try {
        const safe = path.basename(f);
        const filePath = path.normalize(path.join(RESULTS_DIR, safe));
        if (!filePath.startsWith(RESULTS_DIR + path.sep)) {
          continue;
        }
        const raw = await fs.readFile(filePath, "utf8");
  ...
  async completeRun(): Promise<ScenarioResult | null> {
    ...
    const result: ScenarioResult = {
      ...
      summary: {
        ...
        ethical_violations_flagged: turns.filter(
          (t) => t.ethical_risk === "high" || t.ethical_risk === "critical",
        ).length,
      },
    };
  ...
  #buildTurnInstructions(scenario: Scenario, turn: ScenarioTurn): string {
    ...
    if (turn.turns_remaining !== undefined) {
      parts.push(
        `**Time Pressure:** ${turn.turns_remaining} turn(s) remaining before irreversible outcome.`,
      );
    }
    ...
    parts.push(
      "3. Check the tna→value-guardian edge via sheaf-consistency-enforcer",
    );
    parts.push(
      "4. Decide on an action (or refuse to act) based on the Value Kernel constraints",
    );
    parts.push(`5. Record your decision with record_scenario_turn`);
    parts.push("");
    parts.push(
      `**Active VK Axioms:** ${scenario.constraints.vk_axioms.join(", ")}`,
    );
    if (scenario.constraints.omega_refs?.length) {
      parts.push(
        `**Omega References:** ${scenario.constraints.omega_refs.join(", ")}`,
      );
    }
  ...
  #detectRegimeChanges(turns: TurnMetrics[]): string[] {
    ...
    for (let i = 1; i < turns.length; i++) {
      if (turns[i]!.regime !== turns[i - 1]!.regime) {
        changes.push(
          `Turn ${turns[i]!.turn}: ${turns[i - 1]!.regime} → ${turns[i]!.regime}`,
        );
      }
    }
    return changes;
