...
export class SkillRegistry {
    ...
    async initialize() {
        try {
            ...
            console.error('[SkillRegistry] Failed to initialize skills directory:', error);
        }
    }
    ...
    private async loadSkill(folderName: string) {
        ...
        try {
            ...
            console.log(`[SkillRegistry] Skipping ${folderName}, no SKILL.md found or invalid format.`);
        }
    }
        ...
        return [
            {
                ...
                function: {
                    name: "read_skill",
                    description: "Read the full markdown instructions of a specific agent skill.",
                    parameters: {
                        type: "object",
                        properties: {
                            name: {
                                type: "string",
                                description: "The exact name of the skill to read (e.g., 'pdf-processing')."
                            }
                        },
                        required: ["name"]
                    }
