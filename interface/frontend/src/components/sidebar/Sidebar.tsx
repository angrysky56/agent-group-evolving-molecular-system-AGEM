...
export function Sidebar() {
  ...
  const handleNewSession = useCallback(async () => {
    try {
      ...
      setMessages([]);
      // Clear graph/metrics/events so the panel shows a fresh state
  ...
  const handleSelectSession = useCallback(
    async (id: string) => {
      ...
      // Clear stale graph/metrics before loading new session data
      useChatStore.getState().setAgemState(null);
      useAgemStore.getState().resetForSession();
      try {
    ...
    <aside className="sidebar" id="sidebar">
      ...
      <div className="sidebar__section">
        ...
        {sessions.length === 0 ? (
          ...
          >
            No reasoning sessions yet. Click the + button above to start your
            first molecular reasoning cycle.
          </div>
        ) : (
      ...
      <div className="sidebar__section" style={{ flex: "0 0 auto" }}>
        ...
        >
          Workspace directories where AGEM stores detailed semantic analysis and
          output reports.
        </p>
        <div className="knowledge-explorer">
          <div
            className="knowledge-file"
            title="Detailed reasoning reports generated by backend agents."
          >
            <Folder size={12} />
            <span>reports/</span>
          </div>
          <div
            className="knowledge-file"
            title="Topological network analysis (TNA) and sheaf cohomology logs."
          >
            <Folder size={12} />
            <span>analysis/</span>
          </div>
          <div
            className="knowledge-file"
            title="Bridging questions and synthesized concept answers."
          >
            <Folder size={12} />
            <span>outputs/</span>
          </div>
